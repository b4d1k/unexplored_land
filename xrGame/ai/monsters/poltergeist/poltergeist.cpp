...
void CPoltergeist::Load(LPCSTR section)
{
	...
	// skif : (электро-полтергейст)
	else if (xr_strcmp(polter_type, "electric") == 0)
	{
		m_flame = xr_new<CPolterFlame>(this);
		m_flame->load(section);
		((CPolterFlame*)m_flame)->set_hit_type(ALife::eHitTypeTelepatic);
	}
	// end
	else 
	{
...
BOOL CPoltergeist::net_Spawn (CSE_Abstract* DC) 
{
	...
	character_physics_support()->movement()->DestroyCharacter();
	// спаунится нивидимым
	// skif : (выпадение трупа полтергейста)
	if (g_Alive())
		setVisible		(false);
	else
		setVisible		(true);
