...
void CTorch::SwitchNightVision(bool vision_on, bool use_sounds)
{
	...
	if(m_bNightVisionOn)
	{
		...
		if(!bIsActiveNow)
		{
			if(pHelmet && pHelmet->m_NightVisionSect.size() && pHelmet->GetCondition() > 0.25) // skif : (при попадании в шлем отключаем ПНВ)
			{
				m_night_vision->Start(pHelmet->m_NightVisionSect, pA, use_sounds);
				return;
			}
			else if(pOutfit && pOutfit->m_NightVisionSect.size() && pOutfit->GetCondition() > 0.25) // skif : (при попадании в шлем отключаем ПНВ)
			{
...
void CTorch::Switch()
{
	...
	if(pA)						
	{
		CTorchBatarey* bt = smart_cast<CTorchBatarey*>(pA->inventory().ItemFromSlot(TORCH_BATAREY_SLOT));
		if (bt)
			bt->Active(bActive);
		else
			return;
	...
	// skif : (звук при вкл/выкл фонарика)
	LPCSTR snd;
	if (bActive)
		snd = "el_tehnika\\light_switch_off";
	else
		snd = "el_tehnika\\light_switch_on";
	ref_sound s;
	s.create(snd,st_Effect,sg_SourceType);
	s.play_at_pos(0,this->Position());
	// end
	Switch					(bActive);
}
...
CTorchBatarey::CTorchBatarey()
{
	power = 0.f;
	power_reduction_speed = 0.f;
	active = false;
}
...
void CTorchBatarey::Load(LPCSTR section)
{
	inherited::Load(section);

	power_reduction_speed = pSettings->r_float(section,"power_reduction_speed");
	SetPower(pSettings->r_float(section,"power"));
}

void CTorchBatarey::SetPower(float val)
{
	if (val > 100.f)
		val = 100.f;
	if (val < 0.f)
...
void CTorchBatarey::UpdateCL()
{
	inherited::UpdateCL();
}

void CTorchBatarey::Update()
{
	if (!active)
		return;
	SetPower(power-power_reduction_speed);
	
}

void CTorchBatarey::save(NET_Packet &output_packet)
{
	inherited::save	(output_packet);
	save_data		(power,					output_packet);
	save_data		(active,				output_packet);
}

void CTorchBatarey::load(IReader &input_packet)
{
	inherited::load	(input_packet);
	load_data		(power,					input_packet);
	load_data		(active,				input_packet);
}

BOOL CTorchBatarey::net_Spawn(CSE_Abstract* DC) 
{
	return		(inherited::net_Spawn(DC));
}

void CTorchBatarey::net_Destroy() 
{
	inherited::net_Destroy();
}

void CTorchBatarey::OnH_A_Chield() 
{
	inherited::OnH_A_Chield		();
}

void CTorchBatarey::OnH_B_Independent(bool just_before_destroy) 
{
	inherited::OnH_B_Independent(just_before_destroy);
}

void CTorchBatarey::renderable_Render() 
{
	inherited::renderable_Render();
}
