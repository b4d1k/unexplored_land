...
void CInventory::Take(CGameObject *pObj, bool bNotActivate, bool strict_placement)
{
	...
	if(pIItem->CurrPlace()==eItemPlaceUndefined)
	{
		...
		{
			if( CanPutInSlot(pIItem, pIItem->BaseSlot()) && pIItem->BaseSlot() != TORCH_BATAREY_SLOT ) // skif : (батарейка для фонарика)
			{
...
// skif : (панель артефактов на худ)
void CInventory::UpdateArtefactPanel()
{
	CurrentGameUI()->UIMainIngameWnd->HideArtefactPanel();
	u8 afCnt = 0;
	for(TIItemContainer::iterator it = m_belt.begin(); m_belt.end() != it; ++it) 
	{
		CArtefact*	artefact = smart_cast<CArtefact*>(*it);
		if (artefact)
		{
			CurrentGameUI()->UIMainIngameWnd->m_artefact_panel[afCnt]->SetShader(InventoryUtilities::GetEquipmentIconsShader());
			Frect texture_rect;
			texture_rect.x1	= pSettings->r_float(artefact->m_section_id, "inv_grid_x")		*INV_GRID_WIDTH;
			texture_rect.y1	= pSettings->r_float(artefact->m_section_id, "inv_grid_y")		*INV_GRID_HEIGHT;
			texture_rect.x2	= pSettings->r_float(artefact->m_section_id, "inv_grid_width")	*INV_GRID_WIDTH;
			texture_rect.y2	= pSettings->r_float(artefact->m_section_id, "inv_grid_height") *INV_GRID_HEIGHT;
			texture_rect.rb.add(texture_rect.lt);
			CurrentGameUI()->UIMainIngameWnd->m_artefact_panel[afCnt]->SetTextureRect(texture_rect);
			CurrentGameUI()->UIMainIngameWnd->m_artefact_panel[afCnt]->TextureOn();
			CurrentGameUI()->UIMainIngameWnd->m_artefact_panel[afCnt]->SetStretchTexture(true);
			CurrentGameUI()->UIMainIngameWnd->m_artefact_panel[afCnt]->SetWndSize(Fvector2().set(25,25));
			CurrentGameUI()->UIMainIngameWnd->m_artefact_panel[afCnt]->Show(true);
			afCnt++;
		}
	}
}
