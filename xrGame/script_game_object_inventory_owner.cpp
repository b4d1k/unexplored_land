...
void CScriptGameObject::GiveGameNewsRelocateItem(LPCSTR caption, LPCSTR text, LPCSTR texture_name, LPCSTR section, int show_time)
{
	GAME_NEWS_DATA				news_data;
	...
	VERIFY(xr_strlen(texture_name)>0);
	news_data.texture_name			= texture_name;
	Actor()->AddGameNewsRelocateItem(news_data, section);
}

void CScriptGameObject::GiveTalkMessageRelocateItem(LPCSTR caption, LPCSTR text, LPCSTR texture_name, LPCSTR section, LPCSTR templ_name)
{
	CUIGameSP* pGameSP = smart_cast<CUIGameSP*>(CurrentGameUI());
	if(!pGameSP) return;
	...
	if(pGameSP->TalkMenu->IsShown())
	{
		pGameSP->TalkMenu->AddRelocateItemTalkMessage( caption, text, texture_name, section, templ_name ? templ_name : "iconed_answer_item" );
	}
}
// end
