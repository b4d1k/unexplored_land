...
//	xr_strcpy						(name_of_server,*m_caClientOptions);
	...
	g_pGamePersistent->LoadTitle(true, temp);*/
...
bool	CLevel::net_start_client4				()
{
	if(connected_to_server){
		// Begin spawn
		// skif : (вывод стадий загрузки сохранения)
		//g_pGamePersistent->LoadTitle		(true, "st_client_spawning");
		//g_pGamePersistent->LoadTitle		();
		// end
...
bool	CLevel::net_start_client5				()
{
	if(connected_to_server){
		...
		{
			// skif : (вывод стадий загрузки сохранения)
			g_pGamePersistent->LoadTitle		(true, "st_loading_textures");
			//g_pGamePersistent->LoadTitle		();
			// end
...
bool	CLevel::net_start_client6				()
{
	if (connected_to_server) {
		...
		// skif : (вывод стадий загрузки сохранения)
		g_pGamePersistent->LoadTitle		(true, "st_client_synchronising");
		//g_pGamePersistent->LoadTitle		();
		// end
