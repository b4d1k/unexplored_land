	...
	if(m_last_relation==ALife::eRelationTypeEnemy || m_last_relation==ALife::eRelationTypeWorstEnemy)
	{
		...
		if(_object_)
		{
			...
			else
			{
				...
				//skif : (инфракрасный сканер для броников)
				CCustomOutfit* outfit = smart_cast<CCustomOutfit*>(pAct->inventory().ItemFromSlot(OUTFIT_SLOT));
				...
				{
					if (outfit && outfit->m_fShowNearestEnemiesDistance)
					{
						if(pAct->Position().distance_to(pObj->Position()) < outfit->m_fShowNearestEnemiesDistance)
							vis_res = true;
						else
							vis_res = Actor()->memory().visual().visible_now(pObj);
					}
