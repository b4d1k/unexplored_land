...
float fog_distance()
{
	return			(g_pGamePersistent->Environment().CurrentEnv->fog_distance);
}
// end
...
void map_add_iconed_spot(u16 id, LPCSTR spot_type, LPCSTR icon, LPCSTR caption, LPCSTR text, LPCSTR time)
{
	CMapLocation* ml = Level().MapManager().AddMapLocation(spot_type, id);
	ml->m_level_spot->m_icon = icon;
	ml->m_level_spot->m_caption = caption;
	ml->m_level_spot->m_text = text;
	ml->m_level_spot->m_time = time;
	ml->SetSerializable(true);
}
// end
...
void CLevel::script_register(lua_State *L)
{
	...
	[
		...
		// skif : (установки метки на карте с иконкой в описании)
		def("map_add_iconed_spot",				map_add_iconed_spot),
		// end
