...
void CGamePersistent::game_loaded()
{
	if(Device.dwPrecacheFrame<=2)
	{
			...
			m_game_params.m_e_game_type == eGameIDSingle)
		{
			...
			// skif : (вывод стадий загрузки сохранения)
			g_pGamePersistent->LoadTitle(true, "st_press_any_key");
			// end
...
//		tip_num = 83;
		...
		pApp->LoadTitleInt		(CStringTable().translate(map_name.c_str()).c_str(), "", "");
		// end
		/*
		// skif : возможность скриптово отключить вывод "100 советов"
		luabind::functor<bool>	f_set_title;
		R_ASSERT( ai().script_engine().functor( "loadscreen.can_set_tip", f_set_title ) );
		bool can_set_tip = f_set_title();
		if (can_set_tip)
