...
:CEntityConditionSimple()
{
	...
	m_fBoostBurnProtection			= 0.f; //skif : (термозащита у бустеров)
	m_fBoostElectroProtection		= 0.f; //skif : (электрозащита у бустеров)
...
float CEntityCondition::HitPowerEffect(float power_loss)
{
	...
	float power = 0.0f;
	if (pOutfit)
		power += pOutfit->m_fPowerLoss;
	if (helmet)
		power += helmet->m_fPowerLoss;

	return power_loss + (power > 0.0f ? power_loss * power : 0.0f);
	// end
}
...
//		bAddWound		=  is_special_hit_2_self;
	...
	case ALife::eHitTypeShock:
		...
		hit_power -= m_fBoostElectroProtection;
			if(hit_power < 0.f)
				hit_power = 0.f;
		//end		
...
{
	fHealth			= pSettings->r_float(sect.c_str(), "eat_health") / 100;
	fPower			= pSettings->r_float(sect.c_str(), "eat_power") / 100;
	fSatiety		= pSettings->r_float(sect.c_str(), "eat_satiety");
	//skif : (потребности актора)
	fThirst			= pSettings->r_float(sect.c_str(), "eat_thirst") / 100;
	fSleep			= pSettings->r_float(sect.c_str(), "eat_sleep") / 100;
	fBleed			= pSettings->r_float(sect.c_str(), "eat_bleed") / 100;
	//end
	fRadiation		= pSettings->r_float(sect.c_str(), "eat_radiation") / 100;
	...
	fAlcohol		= pSettings->r_float(sect.c_str(), "eat_alcohol") / 100;
	fNausea			= READ_IF_EXISTS	(pSettings, r_float, sect.c_str(),	"nausea", 0.0f);  //skif : (тошнота)
...
void SBooster::Load(const shared_str& sect, EBoostParams type)
{
	...
	switch(type)
	{
		case eBoostHpRestore: fBoostValue = pSettings->r_float(sect.c_str(), "boost_health_restore") / 1000; break;
		case eBoostPowerRestore: fBoostValue = pSettings->r_float(sect.c_str(), "boost_power_restore"); break;
		case eBoostRadiationRestore: fBoostValue = pSettings->r_float(sect.c_str(), "boost_radiation_restore") / 1000; break;
		case eBoostBleedingRestore: fBoostValue = pSettings->r_float(sect.c_str(), "boost_bleeding_restore") / 1000; break;
		...
		case eBoostRadiationProtection: fBoostValue = pSettings->r_float(sect.c_str(), "boost_radiation_protection") / 1000; break;
		case eBoostTelepaticProtection: fBoostValue = pSettings->r_float(sect.c_str(), "boost_telepat_protection") / 1000; break;
		case eBoostChemicalBurnProtection: fBoostValue = pSettings->r_float(sect.c_str(), "boost_chemburn_protection") / 1000; break;
		//skif : (термозащита у бустеров) | (высота прыжков у бустеров) | (восстановление пси-здоровья у бустеров) | (эектрозащита у бустеров)
		case eBoostBurnProtection: fBoostValue = pSettings->r_float(sect.c_str(), "boost_burn_protection") / 1000; break;
		case eBoostElectroProtection: fBoostValue = pSettings->r_float(sect.c_str(), "boost_shock_protection") / 1000; break;
		case eBoostJumpSpeed: fBoostValue = pSettings->r_float(sect.c_str(), "boost_jump_speed") / 10; break;
		case eBoostPsyHpRestore: fBoostValue = pSettings->r_float(sect.c_str(), "boost_psy_health_restore") / 1000; break;
		//end
		default: NODEFAULT;	
	}
}
