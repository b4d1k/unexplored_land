...
CUIActorMenu::CUIActorMenu()
{
	...
	m_ind_shown	= false; //skif : (индикаторы) 
	Construct						();
}
...
void CUIActorMenu::Construct()
{
	...
	// skif : (конвертер дл€ денег)
	m_money_wnd_btn				= UIHelper::Create3tButton(uiXml, "show_money_wnd_button", this);
	m_money_wnd					= xr_new<CUIInvMoneyWnd>(); 
	AttachChild(m_money_wnd);
	m_money_wnd->SetAutoDelete(true);
	m_money_wnd->Init();
...
//	m_PartnerTradeCaption->AdjustWidthToText();
	...
	m_pInventoryTorchBtList		= UIHelper::CreateDragDropListEx(uiXml, "dragdrop_torch_batarey", this);
	// end
	...
	m_Binoc_progress	= UIHelper::CreateProgressBar(uiXml, "progess_bar_binoc", this);
	// end
	...
	//skif : (индикаторы) 
	m_show_indicators_button = UIHelper::Create3tButton(uiXml, "show_indicators_button", this);
	m_indicators_wnd = xr_new<CUIInventoryIndicators>();
	m_indicators_wnd->SetAutoDelete	(true);
	AttachChild (m_indicators_wnd);
...
//-	AttachChild							(m_ItemInfo);
	...
	//end
	// skif : (батарейка дл€ фонарика)
	BindDragDropListEvents				(m_pInventoryTorchBtList);
	// end
...
void CUIActorMenu::InitCallbacks()
{
	...
	//skif : (индикаторы) 
	Register						(m_show_indicators_button);
	//end
	// skif : (конвертер дл€ денег)
	Register						(m_money_wnd_btn);
	// end
	...
	//skif : (индикаторы) 
	AddCallback(m_show_indicators_button,BUTTON_CLICKED,CUIWndCallback::void_function(this, &CUIActorMenu::OnBtnShowIndicatorsClicked));
	//end
	// skif : (конвертер дл€ денег)
	AddCallback(m_money_wnd_btn,BUTTON_CLICKED,CUIWndCallback::void_function(this, &CUIActorMenu::OnBtnShowMoneyClicked));
	//end
}
