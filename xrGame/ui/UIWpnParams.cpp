...
CUIWpnParams::CUIWpnParams()
{
	...
	AttachChild(&m_stAmmoType3); // skif : (отображение всех используемых патрон для оружия)
	AttachChild(&m_stAmmoType4); // skif : (отображение всех используемых патрон для оружия)
}
...
void CUIWpnParams::InitFromXml(CUIXml& xml_doc)
{
	...
	if(IsGameTypeSingle())
	{
		...
		CUIXmlInit::InitStatic			(xml_doc, "wpn_params:static_ammo_type3",	0, &m_stAmmoType3); // skif : (отображение всех используемых патрон для оружия)
		CUIXmlInit::InitStatic			(xml_doc, "wpn_params:static_ammo_type4",	0, &m_stAmmoType4); // skif : (отображение всех используемых патрон для оружия)
	}
...
void CUIWpnParams::SetInfo( CInventoryItem* slot_wpn, CInventoryItem& cur_wpn )
{
	...
	if(IsGameTypeSingle())
	{
		...
		// skif : (отображение всех используемых патрон для оружия)
