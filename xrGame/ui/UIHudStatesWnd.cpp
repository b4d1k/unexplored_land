...
//	m_back_v          = UIHelper::CreateStatic( xml, "back_v", this );
    m_static_armor    = UIHelper::CreateStatic( xml, "progress_bar_armor", this ); // skif : (стамина брони на худе)
	// skif : (батарейка для фонарика)
	m_torch_batarey_state = UIHelper::CreateStatic( xml, "progress_bar_torch_batarey_static", this );
	m_torch_batarey_bar = UIHelper::CreateProgressBar( xml, "progress_bar_torch_batarey", this );
	// end
...
//	m_ui_weapon_icon->Enable	( false );
	...
	m_ui_armor_bar    = UIHelper::CreateProgressBar( xml, "progress_bar_armor", this ); // skif : (стамина брони на худе)
...
//	}
	...
	CTorchBatarey* bt = smart_cast<CTorchBatarey*>(itm_in_torch_bt_slot);
	if (itm_in_torch_slot)
	{
		if (trc->torch_active())
		{
			if (bt)
				m_torch_batarey_bar->SetProgressPos(bt->GetPower());
			else
				m_torch_batarey_bar->SetProgressPos(0.0f);
			m_torch_batarey_bar->Show(true);
			m_torch_batarey_state->Show(true);
		}
		else
		{
			m_torch_batarey_bar->Show(false);
			m_torch_batarey_state->Show(false);
		}
	}
	else
	{
...
*/
	...
	else if(hit_type==ALife::eHitTypeBurn)
	{
	...
	else if(hit_type==ALife::eHitTypeShock)
	{
		it = cur_booster_influences.find(eBoostElectroProtection);
		if(it!=cur_booster_influences.end())
			protect += it->second.fBoostValue;
	}
	//end
