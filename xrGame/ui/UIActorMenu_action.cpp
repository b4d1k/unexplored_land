...
bool CUIActorMenu::OnItemDrop(CUICellItem* itm)
{
	...
	// Зачем нужна эта конструкция? - Чтобы не было возможности вставлять арты в закрытые слоты пояса
	// Получаем Data перетаскиваемого предмета (нам нужен предмет типа belt)
	PIItem	iitem						= (PIItem)itm->m_pData;
	// Если перетаскиваем предмет в слоты пояса (t_new==3)
	if (t_new==3 && iitem->Belt())
	{
		...
		if(belt_cell_pos.y==0 && (belt_cell_pos.x+1)>(m_pActorInvOwner->inventory().BeltWidth()))
			return true;
		if(belt_cell_pos.y==1 && (belt_cell_pos.x+6)>(m_pActorInvOwner->inventory().BeltWidth()))
			return true;
	}

	// Если при перетаскивании тип слота не меняется или переменные старого/нового слота пусты
// Lex Addon (correct by Suhar_) 24.06.2016		(end)
