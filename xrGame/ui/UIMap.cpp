...
void CUILevelMap::SendMessage(CUIWindow* pWnd, s16 msg, void* pData)
{
	...
	if(msg==MAP_SHOW_HINT)
	{
		...
		{	
			...
			if (sp->m_icon != NULL && sp->m_caption != NULL && sp->m_text != NULL && sp->m_time != NULL)
				MapWnd()->ShowHintIconedSpot(sp->m_icon, sp->m_caption, sp->m_text, sp->m_time, sp);
			else
			// end
