...
public:
	...
	CUIStatic*			m_ind_boost_burn; //skif : (термозащита у бустеров)
	CUIStatic*			m_ind_boost_shock; //skif : (электрозащита у бустеров)
	CUIStatic*			m_ind_boost_jump_speed; //skif : (высота прыжков у бустеров)
	CUIStatic*			m_ind_boost_psy_health_restore; //skif : (восстановление пси-здоровья у бустеров)
	// skif : (панель артефактов на худ)
	CUIStatic*			m_artefact_panel[10];
	void 				HideArtefactPanel();
	// end

	// skif - транспорт
	CUIStatic*			m_car_health;
	CUIProgressBar*     m_car_health_progress;
	CUIStatic*			m_car_fuel;
	CUIProgressBar*     m_car_fuel_progress;
	void				ShowCarPanel(bool flag);
	void				SetCarHealth(float health);
	void				SetCarFuel(float fuel);
	// end
...
//		ewiSleep,
	...
	void				ReceiveNewsRelocateItem			(GAME_NEWS_DATA* news, LPCSTR section);
	// end
