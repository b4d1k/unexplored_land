...
void CUIActorMenu::Show(bool status)
{
	...
	if(status)
	{
		...
		m_indicators_wnd->FillIndicators(); //skif : (индикаторы)  
		m_money_wnd->UpdateInfo(); //skif : (конвертер для денег)
	}else
	{
		PlaySnd								(eSndClose);
...
EDDListType CUIActorMenu::GetListType(CUIDragDropListEx* l)
{
	...
	if(l==m_pInventoryTorchBtList)		return iActorSlot;
	// end
...
void CUIActorMenu::highlight_item_slot(CUICellItem* cell_item)
{
	...
	CObject* obj = smart_cast<CObject*>(item);
	CBolt* bolt = smart_cast<CBolt*>(item);
	u8 item_w = pSettings->r_u8(obj->cNameSect(), "inv_grid_width");
...
void CUIActorMenu::ClearAllLists()
{
	...
	//end
	// skif : (батарейка для фонарика)
	m_pInventoryTorchBtList->ClearAll			(true);
	// end
...
bool CUIActorMenu::CanSetItemToList(PIItem item, CUIDragDropListEx* l, u16& ret_slot)
{
	...
	// skif : (слоты)
	if ((item_slot==INV_SLOT_3 && l==m_pInventoryPistolList)||(item_slot==KNIFE_SLOT && l==m_pInventoryPistolList)||(item_slot==BINOCULAR_SLOT && l==m_pInventoryPistolList))
	{
		ret_slot	= INV_SLOT_2;
		return		true;
	}
	...
	if ((item_slot==INV_SLOT_2 && l==m_pInventoryAutomaticList)||(item_slot==KNIFE_SLOT && l==m_pInventoryAutomaticList)||(item_slot==BINOCULAR_SLOT && l==m_pInventoryAutomaticList))
	{
		ret_slot	= INV_SLOT_3;
		return		true;
	}
	
	if ((item_slot==INV_SLOT_2 && l==m_pInventoryKnifeList)||(item_slot==BINOCULAR_SLOT && l==m_pInventoryKnifeList))
	{
		ret_slot	= KNIFE_SLOT;
		return		true;
	}

	if ((item_slot==INV_SLOT_2 && l==m_pInventoryBinocularList)||(item_slot==KNIFE_SLOT && l==m_pInventoryBinocularList))
	{
...
void CUIActorMenu::UpdateConditionProgressBars()
{
	...
	// skif : (слоты)
	CWeaponKnife* knife = smart_cast<CWeaponKnife*>(itm);
	CWeaponBinoculars* binoculars = smart_cast<CWeaponBinoculars*>(itm);
	if(itm && !knife && !binoculars)
	{
	...
	knife = smart_cast<CWeaponKnife*>(itm);
	binoculars = smart_cast<CWeaponBinoculars*>(itm);
	if(itm && !knife && !binoculars)
	...
	binoculars = smart_cast<CWeaponBinoculars*>(itm);
	if(itm && !knife && !binoculars)
		m_Binoc_progress->SetProgressPos(iCeil(itm->GetCondition()*15.0f)/15.0f);
	else
		m_Binoc_progress->SetProgressPos(0);
	// end
...
//skif : (индикаторы)
void CUIActorMenu::OnBtnShowIndicatorsClicked(CUIWindow* w, void* d)
{
	if (!m_ind_shown)
	{
		m_indicators_wnd->Show(true);
		m_ind_shown = true;
		m_indicators_wnd->FillIndicators();
	}
	else
	{
...
void CUIActorMenu::OnBtnShowMoneyClicked(CUIWindow* w, void* d)
{
	m_money_wnd->Show(!m_money_shown);
	m_money_shown = !m_money_shown;
}
//end