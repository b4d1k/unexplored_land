...
:	m_iVal(0),
	...
	m_iMax(100),
	m_iStep(1),
	m_cyclic(false) // skif : (методы GUI-элементов)
{}
...
bool CUISpinNum::CanPressUp()
{
	// skif : (методы GUI-элементов)
	if (IsCyclic())
		return m_iVal + m_iStep <= m_iMax + 1;
	else
...
bool CUISpinNum::CanPressDown()
{
	// skif : (методы GUI-элементов)
	if (IsCyclic())	
		return m_iVal - m_iStep >= m_iMin - 1;
	else
