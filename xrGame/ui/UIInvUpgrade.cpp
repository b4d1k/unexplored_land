...
void UIUpgrade::OnFocusLost()
{
	...
	m_parent_wnd->HighlightHierarchy(get_upgrade()->id(), false, m_state == STATE_ENABLED);
	// end
...
void UIUpgrade::OnClick()
{
	...
	{
		m_parent_wnd->AskUsing(CStringTable().translate("st_upgrade_install").c_str(), get_upgrade()->id_str());
	}
	m_parent_wnd->set_info_cur_upgrade( NULL );
	highlight_relation( true );
...
void UIUpgrade::highlight_relation( bool enable )
{
	if ( enable )
	{
		m_parent_wnd->HighlightHierarchy( get_upgrade()->id(), m_state == STATE_ENABLED, false);
		return;
	}
