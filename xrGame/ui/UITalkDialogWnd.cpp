...
void CUITalkDialogWnd::AddRelocateItemIconedAnswer(LPCSTR caption, LPCSTR text, LPCSTR texture_name, LPCSTR section, LPCSTR templ_name)
{
	CUIAnswerItemIconed* itm		= xr_new<CUIAnswerItemIconed>(m_uiXml,templ_name);
	itm->Init						(text, caption, texture_name);
	itm->m_icon->CreateShader		("ui\\ui_icon_equipment","hud\\default");
	Frect texture_rect;
	...
	itm->m_icon->SetTextureRect(texture_rect);
	itm->m_icon->TextureOn();
	itm->m_icon->SetStretchTexture(true);
	
	itm->m_icon->SetHeight(29.0f);
	...
	{
		d = 41.0f;
		itm->m_icon->SetHeight(41.0f / (w / h));
	}
	itm->m_icon->SetWidth(d);
	Fvector2 pos = itm->m_icon->GetWndPos();
	float x = (41.0f - d) / 2;
	pos.x += x;
	itm->m_icon->SetWndPos(pos);
