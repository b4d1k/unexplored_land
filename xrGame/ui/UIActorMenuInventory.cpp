...
void CUIActorMenu::InitInventoryMode()
{
	...
	//end
	// skif : (батарейка для фонарика)
	m_pInventoryTorchBtList->Show		(true);
	// end
	...
	m_ind_shown = false;
	m_show_indicators_button->Enable(true);
	//end
	// skif : (конвертер для денег)
	m_money_wnd_btn->Enable(true);
	//end
...
void CUIActorMenu::InitInventoryContents(CUIDragDropListEx* pBagList)
{
	...
	//end
	// skif : (батарейка для фонарика)
	InitCellForSlot				(TORCH_BATAREY_SLOT);
	// end
...
bool CUIActorMenu::ToSlot(CUICellItem* itm, bool force_place, u16 slot_id)
{
	...
	PIItem itm_in_torch_slot = inv->ItemFromSlot(TORCH_SLOT);		
	CTorch* trc = smart_cast<CTorch*>(itm_in_torch_slot);
	CTorchBatarey* bt = smart_cast<CTorchBatarey*>(iitem);
	if (!trc && bt)
		return false;
	if (trc && bt)
	{
		trc->m_weight += iitem->m_weight;
		iitem->m_weight = 0.f;
	}
	// end
	...
	if(m_pActorInvOwner->inventory().CanPutInSlot(iitem, slot_id))
	{
		...
		//skif : (запрет перемещения предмета в слот, меньше чем размер предмета)
		if (!new_owner->CanSetItem(itm))
            return false;
		//end
	...
	{ // in case slot is busy
		...
		// skif : (слоты)
		if ( slot_id == KNIFE_SLOT && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_2))
			return ToSlot(itm, force_place, INV_SLOT_2);
			
		if ( slot_id == KNIFE_SLOT && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_3))
			return ToSlot(itm, force_place, INV_SLOT_3);
			
		if ( slot_id == BINOCULAR_SLOT && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_3))
			return ToSlot(itm, force_place, INV_SLOT_3);
			
		if ( slot_id == BINOCULAR_SLOT && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_2))
			return ToSlot(itm, force_place, INV_SLOT_2);
		// end
...
bool CUIActorMenu::ToBag(CUICellItem* itm, bool b_use_cursor_pos)
{
	PIItem	iitem						= (PIItem)itm->m_pData;
	// skif : (батарейка для фонарика)
	CTorch* trc = smart_cast<CTorch*>(iitem);
	if (trc)
	{
		CUIDragDropListEx* torch_bt_slot_list = GetSlotList(TORCH_BATAREY_SLOT);
		if(torch_bt_slot_list->ItemsCount()==1)
		{
			CUICellItem* torch_bt_cell		= torch_bt_slot_list->GetItemIdx(0);
			ToBag(torch_bt_cell, false);
			PIItem item = (PIItem)(torch_bt_cell->m_pData);
			CTorchBatarey* bt = smart_cast<CTorchBatarey*>(item);
			bt->m_weight = pSettings->r_float(bt->cNameSect(),"inv_weight");
		}
	}
	// end
...
CUIDragDropListEx* CUIActorMenu::GetSlotList(u16 slot_idx)
{
	...
	switch ( slot_idx )
	{
		...
		//end
		// skif : (батарейка для фонарика)
		case TORCH_BATAREY_SLOT:
		    return m_pInventoryTorchBtList;
            break;	
		// end
...
bool CUIActorMenu::ToQuickSlot(CUICellItem* itm)
{
	...
	// skif : (запрет перемещать предметы размером больше 1х1 в быстрый слот)
...
void CUIActorMenu::ActivatePropertiesBox()
{
	...
	if ( m_currMenuMode == mmInventory || m_currMenuMode == mmDeadBodySearch)
	{
		...
		CInventory* inv = &m_pActorInvOwner->inventory();
		PIItem itm_in_torch_slot = inv->ItemFromSlot(TORCH_SLOT);		
		PIItem itm_in_torch_bt_slot = inv->ItemFromSlot(TORCH_BATAREY_SLOT);		
		CTorch* trc = smart_cast<CTorch*>(itm_in_torch_slot);
		CTorch* trch = smart_cast<CTorch*>(item);
		if (bt && !itm_in_torch_bt_slot && trc)
		{
			m_UIPropertiesBox->AddItem( "inv_torch_batarey_attach",  NULL, INVENTORY_TORCH_BATAREY_ATTACH );
			b_show			= true;
		}
		if (trc && trch && itm_in_torch_bt_slot)
	    {
			m_UIPropertiesBox->AddItem( "inv_torch_batarey_detach",  NULL, INVENTORY_TORCH_BATAREY_DETACH );
			b_show			= true;
		}
		// end
	}
...
void CUIActorMenu::PropertiesBoxForSlots( PIItem item, bool& b_show )
{
	...
	// skif : (батарейка для фонарика)
	CTorchBatarey* pTorchBatarey = smart_cast<CTorchBatarey*>( item );
	// end
	CInventory&  inv		= m_pActorInvOwner->inventory();
	...
	if ( !pTorchBatarey &&	!pOutfit && !pHelmet && // skif : (батарейка для фонарика)
...
void CUIActorMenu::PropertiesBoxForAddon( PIItem item, bool& b_show )
{
	...
	// skif : (слоты)
	PIItem	item_in_slot_knife = inv->ItemFromSlot(KNIFE_SLOT);
	PIItem	item_in_slot_binoc = inv->ItemFromSlot(BINOCULAR_SLOT);
	// end

	if(!item_in_slot_2 && !item_in_slot_3 && !item_in_slot_knife && !item_in_slot_binoc)	return; // skif : (слоты)
...
//			m_UIPropertiesBox->AddItem( "st_attach_scope_to_rifle",  (void*)item_in_slot_3, INVENTORY_ATTACH_ADDON );
		...
		// skif : (слоты)
		if ( item_in_slot_knife && item_in_slot_knife->CanAttach(pScope) )
		{
			shared_str str = CStringTable().translate("st_attach_scope_to_pistol");
			str.printf("%s %s", str.c_str(), item_in_slot_knife->m_name.c_str());
			m_UIPropertiesBox->AddItem( str.c_str(),  (void*)item_in_slot_knife, INVENTORY_ATTACH_ADDON );
			b_show			= true;
		}
		if ( item_in_slot_binoc && item_in_slot_binoc->CanAttach(pScope) )
		{
			shared_str str = CStringTable().translate("st_attach_scope_to_pistol");
			str.printf("%s %s", str.c_str(), item_in_slot_binoc->m_name.c_str());
			m_UIPropertiesBox->AddItem( str.c_str(),  (void*)item_in_slot_binoc, INVENTORY_ATTACH_ADDON );
			b_show			= true;
		}
		// end
		return;
...
//			m_UIPropertiesBox->AddItem( "st_attach_silencer_to_rifle",  (void*)item_in_slot_3, INVENTORY_ATTACH_ADDON );
		...
		// skif : (слоты)
		if ( item_in_slot_knife && item_in_slot_knife->CanAttach(pSilencer) )
		{
			shared_str str = CStringTable().translate("st_attach_silencer_to_pistol");
			str.printf("%s %s", str.c_str(), item_in_slot_knife->m_name.c_str());
			m_UIPropertiesBox->AddItem( str.c_str(),  (void*)item_in_slot_knife, INVENTORY_ATTACH_ADDON );
		...
		if ( item_in_slot_binoc && item_in_slot_binoc->CanAttach(pSilencer) )
		{
			shared_str str = CStringTable().translate("st_attach_silencer_to_pistol");
			str.printf("%s %s", str.c_str(), item_in_slot_binoc->m_name.c_str());
			m_UIPropertiesBox->AddItem( str.c_str(),  (void*)item_in_slot_binoc, INVENTORY_ATTACH_ADDON );
...
// skif : (чтение текстового описания действия при использовании бустера)
void CUIActorMenu::PropertiesBoxForUsing( PIItem item, bool& b_show )
{
	CEatableItem* pEatItem = smart_cast<CEatableItem*> (item);
	...
	if (pEatItem)
	{
		...
		act_str = READ_IF_EXISTS(pSettings, r_string, section_name, "use_descr", "st_use");
	}
	if ( act_str )
	{
...
void CUIActorMenu::ProcessPropertiesBoxClicked( CUIWindow* w, void* d )
{
	...
	// skif : (батарейка для фонарика)
	case INVENTORY_TORCH_BATAREY_ATTACH: ToSlot( cell_item, true, item->BaseSlot() ); break;
	
	case INVENTORY_TORCH_BATAREY_DETACH: 
		{
			CUIDragDropListEx* torch_bt_slot_list = GetSlotList(TORCH_BATAREY_SLOT);
			if(torch_bt_slot_list->ItemsCount()==1)
			{
				CUICellItem* torch_bt_cell		= torch_bt_slot_list->GetItemIdx(0);
				ToBag(torch_bt_cell, false);
				PIItem itm = (PIItem)(torch_bt_cell->m_pData);
				CTorchBatarey* bt = smart_cast<CTorchBatarey*>(itm);
				bt->m_weight = pSettings->r_float(bt->cNameSect(),"inv_weight");
			}
			CInventory* inv = &m_pActorInvOwner->inventory();
			PIItem itm_in_torch_slot = inv->ItemFromSlot(TORCH_SLOT);		
			CTorch* trc = smart_cast<CTorch*>(itm_in_torch_slot);
			trc->Switch(false);
			trc->m_weight = pSettings->r_float(trc->cNameSect(),"inv_weight");
			break;
		}
	// end
...
void CUIActorMenu::UpdateOutfit()
{
	...
	VERIFY( 0 <= af_count && af_count <= 10 ); //skif : (доп слоты под арты)
