...
bool CUIInventoryCellItem::EqualTo(CUICellItem* itm)
{
	...
	// skif : (айтемы с разным весом не группировать в инвентаре)
	if (object()->m_weight != ci->object()->m_weight)
		return false;
	// end
	
	// skif : (батарейка для фонарика)
	CTorchBatarey* b1 = smart_cast<CTorchBatarey*>(ci->object());
	CTorchBatarey* b2 = smart_cast<CTorchBatarey*>(object());
	if (b1 && b2)
	{
		if (!fsimilar(b1->GetPower(), b2->GetPower(), 0.01f))
			return false;
	}
	// end
