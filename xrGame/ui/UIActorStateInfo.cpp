...
//	m_state[stt_stamina]->init_from_xml( xml, "stamina_state" );
	...
	// skif : (стамины в инвентаре)
	m_state[stt_satiety]->init_from_xml( xml, "satiety_state");
	m_state[stt_thirst]->init_from_xml( xml, "thirst_state");
	m_state[stt_sleep]->init_from_xml( xml, "sleep_state");
	m_state[stt_psyhealth]->init_from_xml( xml, "psyhealth_state");
	m_state[stt_radiodoza]->init_from_xml( xml, "radiodoza_state");
	m_state[stt_bleed]->init_from_xml( xml, "bleed_state");
	m_state[stt_power_now]->init_from_xml( xml, "power_now_state");
	//end
...
//	m_state[stt_main]->init_from_xml( xml, "main_sensor");
	...
	// skif : (стамины в инвентаре)
	m_state[stt_strike]->init_from_xml( xml, "strike_sensor");
	m_state[stt_explosion]->init_from_xml( xml, "explosion_sensor");
	//end
...
// show bleeding icon
	...
	value = floor(value * 55) / 55;
	m_state[stt_psyhealth]->set_progress(value);
	value = actor->conditions().GetRadiation();
	value = floor(value * 55) / 55;
	m_state[stt_radiodoza]->set_progress(value);
	value = actor->conditions().BleedingSpeed();
	value = floor(value * 55) / 55;
	m_state[stt_bleed]->set_progress(value);
	value = actor->conditions().GetPower();
	value = floor(value * 55) / 55;
	m_state[stt_power_now]->set_progress(value);
	//end
	value = actor->conditions().BleedingSpeed();					
...
// show radiation icon
	...
	//skif : (индикаторы защиты в инвентаре в виде цифр)
	...
	m_state[stt_power]->set_text(0.0f);
		// skif : (стамины в инвентаре)
		m_state[stt_strike]->set_text(0.0f);
		m_state[stt_explosion]->set_text(0.0f);
		//end
	//end
	...
	it = cur_booster_influences.find(eBoostBurnProtection);
	if(it!=cur_booster_influences.end())
		burn_value += it->second.fBoostValue;
	//end	
...
//power restore speed progress bar
	{
		...
		m_state[stt_power]->set_text(value);
	}
	// skif : (стамины в инвентаре)
	{
		strk_value += actor->GetProtection_ArtefactsOnBelt(ALife::eHitTypeStrike);
		m_state[stt_strike]->set_text(strk_value);
	}
	
	{
		expl_value += actor->GetProtection_ArtefactsOnBelt(ALife::eHitTypeExplosion);
		m_state[stt_explosion]->set_text(expl_value);
	}
	//end
