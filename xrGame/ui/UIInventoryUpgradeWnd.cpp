...
void CUIInventoryUpgradeWnd::HighlightHierarchy( shared_str const& upgrade_id, bool is_touched, bool need_reload )
{
	UpdateAllUpgrades();
	get_manager().highlight_hierarchy( *m_inv_item, upgrade_id );
	// skif : (подсветка агрейдов одной секции)
	LPCSTR id = get_manager().get_upgrade(upgrade_id)->m_parent_group->id_str();
	if (is_touched)
	{
		UpdateHighlight(id, UIUpgrade::STATE_TOUCHED);
	}
	if (need_reload)
	{
		UpdateHighlight(id, UIUpgrade::STATE_ENABLED);
	}
	// end
}

// skif : (подсветка агрейдов одной секции)
void CUIInventoryUpgradeWnd::UpdateHighlight(LPCSTR id, UIUpgrade::ViewState state)
{
	UI_Upgrades_type::iterator ib = m_current_scheme->cells.begin();
	UI_Upgrades_type::iterator ie = m_current_scheme->cells.end();
	for (; ib != ie; ++ib)
	{
		shared_str iid = (*ib)->get_upgrade()->id();
		LPCSTR cid = get_manager().get_upgrade(iid)->m_parent_group->id_str();
		Upgrade_type* i_upgr = (*ib)->get_upgrade();
		UIUpgrade* uiu = FindUIUpgrade(i_upgr);
		if (id == cid)
		{
			uiu->set_texture(UIUpgrade::LAYER_COLOR, get_cell_texture(state));
		}
	}
}
// end
