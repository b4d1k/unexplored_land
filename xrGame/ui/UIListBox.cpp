...
// skif : (методы GUI-элементов)
CUIListBoxItem* CUIListBox::AddRelationItem(LPCSTR text, u32 color, LPCSTR icon)
{
	CUIListBoxItem* pItem			= AddItem();

	m_def_item_height = 52.0f;
	pItem->SetWndSize				(Fvector2().set(GetDesiredChildWidth(), m_def_item_height));
	pItem->SetTextColor				(color);
	pItem->SetText					(CStringTable().translate(text).c_str());
	pItem->GetTextItem()->SetWidth	(GetDesiredChildWidth());
	pItem->GetTextItem()->SetWndPos (Fvector2().set(128.0f,0.0f));
	CUIStatic* st = xr_new<CUIStatic>();
	st->SetAutoDelete		(true);
	st->SetWndPos			(Fvector2().set(0.0f,0.0f));
	st->SetWndSize			(Fvector2().set(126.0f, 52.0f));
	st->InitTexture(icon);
	pItem->AttachChild(st);
	return pItem;
}
// end
