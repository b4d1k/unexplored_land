...
class CUIInventoryIndicators; //skif : (индикаторы)
...
protected:
	...
	CUIDragDropListEx* 			m_pInventoryTorchBtList;
	// end
	...
	enum						{e_af_count = 10}; //skif : (доп слоты под арты)
	...
	CUIProgressBar*				m_Binoc_progress;
	// end
...
//	CUITextWnd*					m_ActorTradeCaption;
	...
	//skif : (индикаторы) 
	CUI3tButton*				m_show_indicators_button; 
	CUIInventoryIndicators*		m_indicators_wnd;
	bool						m_ind_shown; 
	//end
	//skif : (конвертер для денег)
	CUI3tButton*				m_money_wnd_btn;
	CUIInvMoneyWnd*				m_money_wnd;
	bool						m_money_shown;
	//end
...
public:
	...
	void		xr_stdcall		OnBtnShowIndicatorsClicked	(CUIWindow* w, void* d); //skif : (индикаторы)
	void		xr_stdcall		OnBtnShowMoneyClicked		(CUIWindow* w, void* d); // skif : (конвертер для денег)
