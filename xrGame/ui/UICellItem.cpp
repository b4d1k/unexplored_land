...
// skif : (не отображать шкалу изношенности у ножа и бинокля)
...
//-	m_mark				= NULL;
	m_upgrade			= NULL;
	m_uniq_mark			= NULL; // skif : (метка на иконке для уников)
...
/*	m_mark					= xr_new<CUIStatic>();
	...
	// skif : (метка на иконке для уников)
	m_uniq_mark				= xr_new<CUIStatic>();
	m_uniq_mark->SetAutoDelete( true );
	AttachChild				( m_uniq_mark );
	CUIXmlInit::InitStatic	( uiXml, "cell_item_unique", 0, m_uniq_mark );
	m_uniq_mark_pos			= m_uniq_mark->GetWndPos();
	m_uniq_mark->Show			( false );
	// end
...
//		pos.x = size.x - up_size.x - 4.0f;
		...
		// skif : (метка на иконке для уников)			
			...
			is_unique = READ_IF_EXISTS(pSettings, r_bool, section_name, "unique_item", false);
							
			pos.set( m_uniq_mark_pos );
			if ( ChildsCount() )
			{
				pos.x += m_text->GetWndSize().x + 4.0f;
			}
			m_uniq_mark->SetWndPos( pos );
		// end
	}
	m_upgrade->Show( m_has_upgrade );
	m_uniq_mark->Show( is_unique );  // skif : (метка на иконке для уников) 
}
...
void CUICellItem::UpdateConditionProgressBar()
{
	...
	if(m_pParentList && m_pParentList->GetConditionProgBarVisibility())
	{
		...
		// skif : (не отображать шкалу изношенности у ножа и бинокля)
		CWeaponKnife* pKnife = smart_cast<CWeaponKnife*>(itm);
		CWeaponBinoculars* pBinoculars = smart_cast<CWeaponBinoculars*>(itm);
		//end
		//if (pKnife || pBinoculars) return; // skif : (не отображать шкалу изношенности у ножа и бинокля)
		
		if((pWeapon || pOutfit || pHelmet) && !pKnife && !pBinoculars) // skif : (не отображать шкалу изношенности у ножа и бинокля)
		{
