...
//-	delete_data( pUIFactionWarWnd );
	...
	// skif : (разделы в ПДА)
	delete_data( pUIEncWnd );
	delete_data( pUIRelationWnd );
	// end 
...
*/
	...
	// skif : (разделы в ПДА)
	m_btn_enc				= UIHelper::Create3tButton( uiXml, "enc_button", this ); 
	m_btn_rel				= UIHelper::Create3tButton( uiXml, "rel_button", this ); 
	// end
...
//-		pUIFactionWarWnd->Init			();
		...
		// skif : (разделы в ПДА)
		pUIEncWnd					= xr_new<CUIEncWnd>();
		pUIEncWnd->Init();
		
		pUIRelationWnd				= xr_new<CUIRelationWnd>();
		pUIRelationWnd->Init();
		pUIRelationWnd->UpdateInfo();
		// end
	...
	case BUTTON_CLICKED:
		{
			if ( pWnd == m_btn_close )
				HideDialog();
			// skif : (разделы в ПДА)
			else if ( pWnd == m_btn_enc )
				SetActiveSubdialog("eptEnc");
			else if ( pWnd == m_btn_rel )
				SetActiveSubdialog("eptRelation");
			//end 
			break;
		}
	default:
...
//-	}
	...
	// skif : (разделы в ПДА)
		...
		m_pActiveDialog = pUIEncWnd;
	else if ( section == "eptRelation" )
	{
		m_pActiveDialog = pUIRelationWnd;
		pUIRelationWnd->UpdateInfo();
	}
	// end
...
//-	}
	...
	// skif : (разделы в ПДА)
	/*else if ( m_pActiveDialog == pUIRelationWnd )
	{
		pUIRelationWnd->UpdateInfo();
	}*/
	// end
...
//-	if ( pUIFactionWarWnd )	pUITaskWnd->ResetAll();
	...
	// skif : (разделы в ПДА)
	if ( pUIEncWnd )		pUIEncWnd->ResetAll();
	if ( pUIRelationWnd )	pUIRelationWnd->ResetAll();
	// end
}
