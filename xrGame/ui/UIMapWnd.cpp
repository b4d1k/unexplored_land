...
// skif : (отображение локаций в ПДА по инфопоршню)
void CUIMapWnd::UpdatePdaMaps()
{
	...
	if (pGameIni->section_exist(sect_name.c_str()))
	{
		...
		for (; it != end; it++)
		{
			...
			CUICustomMap*& l = m_GameMaps[map_name];
			LPCSTR infoportion;
			STRCONCAT(infoportion, map_name, "_map_show");
			if (!Actor()->HasInfo(infoportion))
				l->Show(false);
		}
	}
}
...
void CUIMapWnd::Update()
{
	...
	UpdateNav					();
	UpdatePdaMaps				(); // skif : (отображение локаций в ПДА по инфопоршню)
}
...
// skif : (установки метки на карте с иконкой в описании)
void CUIMapWnd::ShowHintIconedSpot(LPCSTR icon, LPCSTR caption, LPCSTR text, LPCSTR time, CUIWindow* owner)
{
	m_map_location_hint->SetInfo(icon, caption, text, time);
	m_map_location_hint->SetOwner(owner);
	ShowHint(true);
}
// end
