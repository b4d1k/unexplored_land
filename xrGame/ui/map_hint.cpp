...
void CUIMapLocationHint::SetInfo(LPCSTR icon, LPCSTR caption, LPCSTR text, LPCSTR time)
{
	SetInfoMode(2);
	CUIStatic* S = m_info["t_icon"];
	...
	S = m_info["t_caption"];
	S->TextItemControl()->SetTextST(caption);
	S->AdjustHeightToText();

	S = m_info["t_time"];
	S->TextItemControl()->SetText(time);
	...
	m_info["t_time_rem"]->Show(false);
	m_info["t_icon"]->Show(true);
	float w = m_info["t_time"]->GetWidth();

	pos = m_info["t_icon"]->GetWndPos();
	pos.x = m_posx_icon;
	m_info["t_icon"]->SetWndPos(pos);
