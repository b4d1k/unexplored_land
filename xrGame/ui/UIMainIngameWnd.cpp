...
#include "../Artefact.h"	// skif : (панель артефактов на худ)
...
*/	//---------------------------------------------------------
	...
	m_ind_sleep				= UIHelper::CreateStatic(uiXml, "indicator_sleep", this);
	//end
	m_ind_boost_burn		= UIHelper::CreateStatic(uiXml, "indicator_booster_burn", this); //skif : (термозащита у бустеров)
	m_ind_boost_shock		= UIHelper::CreateStatic(uiXml, "indicator_booster_shock", this); //skif : (электрозащита у бустеров)
	m_ind_boost_jump_speed	= UIHelper::CreateStatic(uiXml, "indicator_booster_jump_speed", this); //skif : (высота прыжков у бустеров)
	m_ind_boost_psy_health_restore		= UIHelper::CreateStatic(uiXml, "indicator_booster_psy_health_restore", this); //skif : (восстановление пси-здоровья у бустеров)
	...
	m_ind_boost_burn		->Show(false); //skif : (термозащита у бустеров)
	m_ind_boost_shock		->Show(false); //skif : (электрозащита у бустеров)
	m_ind_boost_jump_speed	->Show(false); //skif : (высота прыжков у бустеров)
	m_ind_boost_psy_health_restore	->Show(false); //skif : (восстановление пси-здоровья у бустеров)
	// skif : (панель артефактов на худ)
	for (u8 i=0; i<10; i++)
	{
		string32 str = "";
		xr_sprintf( str, "artefact_panel_%u", i+1);
		m_artefact_panel[i] = UIHelper::CreateStatic(uiXml, str, this);
		m_artefact_panel[i]->Show(false);
	}
	// end

	// skif - транспорт
	m_car_health = UIHelper::CreateStatic(uiXml, "car_health_static", this);
	m_car_health_progress = UIHelper::CreateProgressBar(uiXml, "car_health_progress", this);
	m_car_health->Show(false);
	m_car_health_progress->Show(false);
	m_car_fuel = UIHelper::CreateStatic(uiXml, "car_fuel_static", this);
	m_car_fuel_progress = UIHelper::CreateProgressBar(uiXml, "car_fuel_progress", this);
	m_car_fuel->Show(false);
	m_car_fuel_progress->Show(false);
	// end
...
// skif : (панель артефактов на худ)
void CUIMainIngameWnd::HideArtefactPanel()
{
	for (u8 i=0; i<10; i++)
		m_artefact_panel[i]->Show(false);
}
...
// skif - транспорт
void CUIMainIngameWnd::ShowCarPanel(bool flag)
{
	m_car_health->Show(flag);
	m_car_health_progress->Show(flag);
	m_car_fuel->Show(flag);
	m_car_fuel_progress->Show(flag);
}

void CUIMainIngameWnd::SetCarHealth(float health)
{
	m_car_health_progress->SetProgressPos(health * 100.0f);
}

void CUIMainIngameWnd::SetCarFuel(float fuel)
{
	m_car_fuel_progress->SetProgressPos(fuel);
}
// end
...
void CUIMainIngameWnd::Draw()
{
	...
	// skif : (панель артефактов на худ)
	pActor->inventory().UpdateArtefactPanel();
	// end
	RenderQuickInfos();		
}
...
void CUIMainIngameWnd::ReceiveNewsRelocateItem(GAME_NEWS_DATA* news, LPCSTR section)
{
	VERIFY(news->texture_name.size());

	CurrentGameUI()->m_pMessagesWnd->AddIconedPdaMessageRelocateItem(news, section);
	CurrentGameUI()->UpdatePda();
}
// end
...
// Bleeding icon
	...
	else
	{
		...
		else if(bleeding<=0.75f)
		{
			m_ind_bleeding->InitTexture("ui_inGame2_circle_bloodloose_orange");
			m_ind_bleeding->SetColorAnimation("ui_medium_blinking_alpha", flags);
		}
		else
		{
...
// Radiation icon
	...
	else
	{
		...
		else if(radiation<=0.75f)
		{
			m_ind_radiation->InitTexture("ui_inGame2_circle_radiation_orange");
			m_ind_radiation->SetColorAnimation("ui_medium_blinking_alpha", flags);
		}
		else
		{
...
// Satiety icon
	...
	else
	{
		...
		if(satiety_koef>-0.25f)
			...
			m_ind_starvation->InitTexture("ui_inGame2_circle_hunger_orange");
		else
	...
	else
	{
		...
		if(thirst_koef>-0.25f)
			m_ind_thirst->InitTexture("ui_inGame2_circle_thirst_green");
		else if(thirst_koef>-0.45f)
			m_ind_thirst->InitTexture("ui_inGame2_circle_thirst_yellow");
		else if(thirst_koef>-0.55f)
			m_ind_thirst->InitTexture("ui_inGame2_circle_thirst_orange");
		else
	...
	if(sleep_koef>0.75)
		m_ind_sleep->Show(false);
	else
	{
		m_ind_sleep->Show(true);
		if(sleep_koef<=0.75f)
			m_ind_sleep->InitTexture("ui_inGame2_circle_sleep_green");
		else if(sleep_koef<=0.5f)
			m_ind_sleep->InitTexture("ui_inGame2_circle_sleep_yellow");
		else if(sleep_koef<=0.25f)
			m_ind_sleep->InitTexture("ui_inGame2_circle_sleep_orange");
		else
			m_ind_sleep->InitTexture("ui_inGame2_circle_sleep_red");
	}
	//end
// Armor broken icon
...
// Weapon broken icon
	...
	// skif : (слоты)
	if(slot==INV_SLOT_3 || slot==INV_SLOT_2 || slot==KNIFE_SLOT || slot==BINOCULAR_SLOT)
	// end
	{
		...
		if(weapon)
		{
			...
			if(condition<start_misf_cond)
			{
				...
				else if(condition>(end_misf_cond/2))
					m_ind_weapon_broken->InitTexture("ui_inGame2_circle_Gunbroken_yellow");
				else if(condition>end_misf_cond)
					m_ind_weapon_broken->InitTexture("ui_inGame2_circle_Gunbroken_orange");
				else
...
void CUIMainIngameWnd::DrawMainIndicatorsForInventory()
{
	...
	//skif : (термозащита у бустеров) | (высота прыжков у бустеров) | (восстановление пси-здоровья у бустеров) | (электрозащита у бустеров)
	if(m_ind_boost_burn->IsShown())
	{
		m_ind_boost_burn->Update();
		m_ind_boost_burn->Draw();
	}
	if(m_ind_boost_shock->IsShown())
	{
		m_ind_boost_shock->Update();
		m_ind_boost_shock->Draw();
	}
	
	if(m_ind_boost_jump_speed->IsShown())
	{
		m_ind_boost_jump_speed->Update();
		m_ind_boost_jump_speed->Draw();
	}
	if(m_ind_boost_psy_health_restore->IsShown())
	{
		m_ind_boost_psy_health_restore->Update();
		m_ind_boost_psy_health_restore->Draw();
	}
	//end
	...
	for(; b!=e; b++)
	{
		switch(b->second.m_type)
		{
			...
			case eBoostChemicalBurnProtection: 
				...
				break;
			//skif : (термозащита у бустеров) | (высота прыжков у бустеров) | (восстановление пси-здоровья у бустеров) | (электрозащита у бустеров)
			case eBoostBurnProtection: 
				{
					m_ind_boost_burn->Show(true); 
					if(b->second.fBoostTime<=3.0f)
						m_ind_boost_burn->SetColorAnimation(str_flag, flags);
					else
						m_ind_boost_burn->ResetColorAnimation();
				}
				break;
			case eBoostElectroProtection: 
				{
					m_ind_boost_shock->Show(true); 
					if(b->second.fBoostTime<=3.0f)
						m_ind_boost_shock->SetColorAnimation(str_flag, flags);
					else
						m_ind_boost_shock->ResetColorAnimation();
				}
				break;
				
			case eBoostJumpSpeed: 
				{
					m_ind_boost_jump_speed->Show(true); 
					if(b->second.fBoostTime<=3.0f)
						m_ind_boost_jump_speed->SetColorAnimation(str_flag, flags);
					else
						m_ind_boost_jump_speed->ResetColorAnimation();
				}
				break;	
				
			case eBoostPsyHpRestore: 
				{
					m_ind_boost_psy_health_restore->Show(true); 
					if(b->second.fBoostTime<=3.0f)
						m_ind_boost_psy_health_restore->SetColorAnimation(str_flag, flags);
					else
						m_ind_boost_psy_health_restore->ResetColorAnimation();
				}
				break;	
			//end
		}
