	...
	virtual void	load				(LPCSTR sect, LPCSTR prefix)
	{
		...
		do{
			...
			if(pSettings->line_exist(sect,temp))
			{
				...
				// skif : (детекторы) - если нет для арта параметров sound и freq - берем дефолтные
				// чтоб меньше текста в секции детектора
				...
				if (!pSettings->line_exist(sect,temp))
					HUD_SOUND_ITEM::LoadSound(sect, "detectors\\art_beep1", item_type.detect_snds, SOUND_TYPE_ITEM);
				else
