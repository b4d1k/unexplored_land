	...
	case kWPN_FIRE:
		{
			...
			// skif : (слоты)
			if(inventory().ActiveItem() && (slot==INV_SLOT_3 || slot==INV_SLOT_2 || slot==KNIFE_SLOT || slot==BINOCULAR_SLOT || slot==BOLT_SLOT) )
			// end
...
void CActor::SwitchNightVision()
{
	...
	if(inventory().ItemFromSlot(KNIFE_SLOT))
		wpn3 = smart_cast<CWeapon*>(inventory().ItemFromSlot(KNIFE_SLOT));

	if(inventory().ItemFromSlot(BINOCULAR_SLOT))
		wpn4 = smart_cast<CWeapon*>(inventory().ItemFromSlot(BINOCULAR_SLOT));
	// end
