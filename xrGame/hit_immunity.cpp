...
// skif
void CHitImmunity::LoadAfsImmunities(LPCSTR imm_sect, CInifile const * ini)
{
	R_ASSERT2	(ini->section_exist(imm_sect), imm_sect);

	m_HitImmunityKoefs[ALife::eHitTypeBurn]			= ini->r_float(imm_sect,"burn_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeStrike]		= ini->r_float(imm_sect,"strike_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeShock]		= ini->r_float(imm_sect,"shock_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeWound]		= ini->r_float(imm_sect,"wound_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeRadiation]	= ini->r_float(imm_sect,"radiation_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeTelepatic]	= ini->r_float(imm_sect,"telepatic_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeChemicalBurn] = ini->r_float(imm_sect,"chemical_burn_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeExplosion]	= ini->r_float(imm_sect,"explosion_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeFireWound]	= ini->r_float(imm_sect,"fire_wound_immunity")/100;
	m_HitImmunityKoefs[ALife::eHitTypeLightBurn]	= m_HitImmunityKoefs[ALife::eHitTypeBurn];
}

void CHitImmunity::LoadOutfitImmunities(LPCSTR imm_sect, CInifile const * ini)
{
	R_ASSERT2	(ini->section_exist(imm_sect), imm_sect);
