...
void CChangeLevelWnd::OnOk()
{
	HideDialog								();
	// skif - (транспорт)
	if (Actor()->IsActorInVehicle)
	{
		CHolderCustom* h = Actor()->m_holder;
		CCar*	car = smart_cast<CCar*>(h);
		/*LPCSTR sectCar = car->cNameSect_str();
		float healthCar = car->GetfHealth();
		float fuelCar = car->GetFuel();*/
		u16 carId = car->ID();
		//Actor()->detach_Vehicle();
		NET_Packet				net_packet;
		Fvector pos;
		pos.x = m_position.x;
		pos.y = m_position.y + 3;
		pos.z = m_position.z;
		GameGraph::_GRAPH_ID	_game_vertex_id = (GameGraph::_GRAPH_ID)m_game_vertex_id;
		car->cast_script_entity()->object().u_EventGen(net_packet, GE_TELEPORT_OBJECT, carId);
		net_packet.w(&_game_vertex_id, sizeof(_game_vertex_id));
		net_packet.w(&m_level_vertex_id, sizeof(m_level_vertex_id));
		net_packet.w_vec3(pos);
		Level().Send(net_packet, net_flags(TRUE, TRUE));
		//car->DestroyObject();
		/*luabind::functor<void>	functor;
		R_ASSERT(ai().script_engine().functor("bind_stalker.set_car_on_target_level", functor));
		functor(carId);*/
		luabind::functor<void>	functor;
		R_ASSERT(ai().script_engine().functor("bind_stalker.show_target_level", functor));
		functor(m_position.x, m_position.y, m_position.z, m_level_vertex_id, m_game_vertex_id);
	}
	// end
...
// skif : (восстановления окна Да/Нет для level_changer)
