...
#define MAX_SLEEP					1.0f
#define START_SLEEP					0.5f
//end
...
void CActorCondition::LoadCondition(LPCSTR entity_section)
{
	...
	m_fV_Nausea					= pSettings->r_float(section,"nausea_v"); //skif : (тошнота)
	...
	m_fSleepCritical			= pSettings->r_float(section,"sleep_critical");
	clamp						(m_fThirstCritical, 0.0f, 1.0f);
	m_fV_Sleep					= pSettings->r_float(section,"sleep_v");		
	m_fV_SleepPower				= pSettings->r_float(section,"sleep_power_v");
	m_fV_SleepHealth			= pSettings->r_float(section,"sleep_health_v");
...
void CActorCondition::UpdateCondition()
{
	if(psActorFlags.test(AF_GODMODE_RT))
	{
		...
		clamp			(m_fSleep,			0.0f,		1.0f);
			CEffectorCam* ce2 = Actor()->Cameras().GetCamEffector((ECamEffectorType)effActorSleep);
			if(ce2)
				RemoveEffector(m_object,effActorSleep);
		//end
		
		//skif : (тошнота)		
		m_fNausea		+= m_fV_Nausea*m_fDeltaTime;
		clamp			(m_fNausea,			0.0f,		1.0f);
			CEffectorCam* ce3 = Actor()->Cameras().GetCamEffector((ECamEffectorType)effNausea);
			if(ce3)
				RemoveEffector(m_object,effNausea);
		//end
	}
	...
	{	
		...
		if	((m_fSleep<0.2f) ){
			if(!ce2){
				AddEffector(m_object,effActorSleep, "effector_actor_sleep", GET_KOEFF_FUNC(this, &CActorCondition::GetSleep));
			}
		}else{
			if(ce2)
				RemoveEffector(m_object,effActorSleep);
		...
		//skif : (тошнота)
		CEffectorCam* ce3 = Actor()->Cameras().GetCamEffector((ECamEffectorType)effNausea);
		if	((m_fNausea>0.0001f) ){
			if(!ce3){
				AddEffector(m_object,effNausea, "effector_nausea", GET_KOEFF_FUNC(this, &CActorCondition::GetNausea));
			}
		}else{
			if(ce3)
				RemoveEffector(m_object,effNausea);
...
void CActorCondition::save(NET_Packet &output_packet)
{
	...
	//end
	save_data			(m_fNausea, output_packet); //skif : (тошнота)
	...
	//end
	save_data			(m_curr_medicine_influence.fNausea, output_packet); //skif : (тошнота)
...
void CActorCondition::load(IReader &input_packet)
{
	...
	//end
	load_data			(m_fNausea, input_packet); //skif : (тошнота)
	...
	load_data			(m_curr_medicine_influence.fNausea, input_packet); //skif : (тошнота)
...
//skif : (тошнота)
void CActorCondition::ChangeNausea	(float value)
{
	m_fNausea += value;
}
//end
...
void CActorCondition::ChangeSatietySpeed(float value)
{
	m_fV_Satiety -= value;
	clamp		(m_fV_Satiety, 0.0f, 1.0f);
}

void CActorCondition::ChangeThirstSpeed(float value)
{
	m_fV_Thirst -= value;
	clamp		(m_fV_Thirst, 0.0f, 1.0f);
}

void CActorCondition::ChangeSleepSpeed(float value)
{
	m_fV_Sleep -= value;
	clamp		(m_fV_Sleep, 0.0f, 1.0f);
}
// end
...
void CActorCondition::BoostParameters(const SBooster& B)
{
	if(OnServer())
	{
		switch(B.m_type)
		{
			...
			case eBoostBurnProtection: BoostBurnProtection(B.fBoostValue); break; //skif : (термозащита у бустеров)
			case eBoostElectroProtection: BoostElectroProtection(B.fBoostValue); break; //skif : (электрозащита у бустеров)
			case eBoostJumpSpeed: BoostJumpSpeed(B.fBoostValue); break; //skif : (высота прыжков у бустеров)
			case eBoostPsyHpRestore: BoostPsyHpRestore(B.fBoostValue); break; //skif : (восстановление пси-здоровья у бустеров)
			default: NODEFAULT;	
		}
	}
}
...
//skif : (восстановление пси-здоровья у бустеров)
void CActorCondition::BoostPsyHpRestore(const float value)
{
	m_change_v.m_fV_PsyHealth += value;
}
//end

//skif : (термозащита у бустеров)
...
//skif : (электрозащита у бустеров)
void CActorCondition::BoostElectroProtection(const float value)
{
	m_fBoostElectroProtection += value;
}
//end

//skif : (высота прыжков у бустеров)
void CActorCondition::BoostJumpSpeed(const float value)
{
	float speed = object().m_fJumpSpeed;
	float speed_act = object().m_fJumpSpeed;
	BOOSTER_MAP::iterator it = m_booster_influences.find(eBoostJumpSpeed);
	speed += value;
	if(it->second.fBoostTime>0.0f)
		{ object().character_physics_support()->movement()->SetJumpUpVelocity(speed); }
		else
		{ object().character_physics_support()->movement()->SetJumpUpVelocity(speed_act); }
}
//end
...
void CActorCondition::UpdateTutorialThresholds()
{
	...
	static float _cSleep			= pSettings->r_float("tutorial_conditions_thresholds","sleep");
	//end
	...
	if(b && !m_condition_flags.test(eCriticalSleepReached) && GetSleep()<_cSleep){
		m_condition_flags.set			(eCriticalSleepReached, TRUE);
		b=false;
		xr_strcpy(cb_name,"_G.on_actor_sleep");
	}
	//end
