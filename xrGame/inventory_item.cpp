...
void CInventoryItem::Load(LPCSTR section) 
{
	...
	// skif
	if (sl == 6 || sl == 11)
		CHitImmunity::LoadOutfitImmunities	(pSettings->r_string(section,"immunities_sect"),pSettings);
	else
	...
	if ( BaseSlot() != NO_ACTIVE_SLOT || Belt())
	{
		...
		// skif : (рандомный вес у артефактов)
		if (Belt()) 
		{
			bool rnd_af_w = pSettings->r_bool(section, "random_weight");
			if (rnd_af_w)
			{
				m_weight += 0.05f * (Random.randI(10) - 5);
				if (m_weight <= 0.0f) 
					m_weight = 0.05f;
			}
		}
...
//процесс отсоединени€ вещи заключаетс€ в спауне новой вещи 
//в инвентаре и установке соответствующих флагов в родительском
//объекте, поэтому функци€ должна быть переопределена
...
void CInventoryItem::save(NET_Packet &packet)
{
	...
	// skif : (сохранение параметра m_weight у экземпл€ров класса CInventoryItem)
	CArtefact* af = smart_cast<CArtefact*>(this);
	if (af)
	{
		packet.w_u8(1);
