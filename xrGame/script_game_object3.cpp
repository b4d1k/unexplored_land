...
float CScriptGameObject::GetCurrentOutfitCamuflageCoeff()
{
	CInventoryOwner		*inventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!inventoryOwner) {
		ai().script_engine().script_log		(ScriptStorage::eLuaMessageTypeError,"CInventoryOwner : cannot access class member GetCurrentOutfitCamuflageCoeff!");
		return		(0);
	}
	CGameObject		*current_equipment = inventoryOwner->GetOutfit();
	CCustomOutfit* o = smart_cast<CCustomOutfit*>(current_equipment);
	...
	return		o->OutfitCamuflageCoeff();
}
// end
// skif : (ф-ции для скриптов)
LPCSTR CScriptGameObject::GetInvName()
{
	CInventoryItem		*inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item)
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CInventoryOwner : cannot access class member GetInvName!");
		return "";
	}
	return inventory_item->NameItem();
}

LPCSTR CScriptGameObject::GetInvDescr()
{
	CInventoryItem		*inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item)
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CInventoryOwner : cannot access class member GetInvDescr!");
		return "";
	}
	return inventory_item->ItemDescription().c_str();
}

LPCSTR CScriptGameObject::GetInvNameBySection(LPCSTR section)
{
	LPCSTR name = pSettings->r_string(section, "inv_name");
	return name;
}

LPCSTR CScriptGameObject::GetInvDescrBySection(LPCSTR section)
{
	LPCSTR descr = pSettings->r_string(section, "description");
	return descr;
}
// end
