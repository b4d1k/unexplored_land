...
void	CCar::Load					( LPCSTR section )
{
	...
	m_fuel_tank = pSettings->r_float(section, "fuel_tank");
...
BOOL	CCar::net_Spawn				(CSE_Abstract* DC)
{
	...
	/*ALife::OBJECT_IT		i = DC->children.begin();
	ALife::OBJECT_IT		j = DC->children.end();

	for (; i != j; ++i)
	{
		CSE_ALifeDynamicObject *ce = ai().alife().objects().object(*i);
		if (ce)
		{
			CObject *obj = smart_cast<CObject*>(ce);
			if (obj)
			{
...
void CCar::SaveNetState(NET_Packet& P)
{
	...
	P.w_u16((u16)m_trunk_items.size());
	for (xr_vector<u16>::const_iterator it = m_trunk_items.begin(); m_trunk_items.end() != it; ++it)
	{
		P.w_u16(*it);
	}
}
...
void CCar::RestoreNetState(CSE_PHSkeleton* po)
{
	...
	/*m_trunk_items.clear();
	for (xr_vector<u16>::const_iterator it = co->trunk_items.begin(); co->trunk_items.end() != it; ++it)
	{
		u16 id = *it;
		luabind::functor<void>	functor;
		R_ASSERT(ai().script_engine().functor("main.m_trunk_items", functor));
		functor(*it);
...
////////////////////////////////////////////////////////////////////
	...
	PPhysicsShellHolder()->ForceTransform(replace);
 ...
 void CCar::VisualUpdate(float fov)
{
	...
	if(Owner())
	{
		...
		if (CurrentGameUI())
		{
			CurrentGameUI()->UIMainIngameWnd->ShowCarPanel(true);
			CurrentGameUI()->UIMainIngameWnd->SetCarHealth(GetfHealth());
			CurrentGameUI()->UIMainIngameWnd->SetCarFuel((GetFuel()/GetFuelTank()) * 100);
		}
// 
// 		if(OwnerActor() && OwnerActor()->IsMyCamera()) 
// 		{
...
void CCar::detach_Actor()
{
	...
	if (CurrentGameUI())
	{
		CurrentGameUI()->UIMainIngameWnd->ShowCarPanel(false);
	}
...
bool CCar::Use(const Fvector& pos,const Fvector& dir,const Fvector& foot_pos)
{
	...
	/*luabind::functor<void>	functor;
	R_ASSERT(ai().script_engine().functor("bind_car.use_car", functor));
	functor();*/

	CUICarWnd* m_car_wnd = xr_new<CUICarWnd>();
	m_car_wnd->Init(this);
	m_car_wnd->ShowDialog(true);
...
void CCar::OnEvent(NET_Packet& P, u16 type)
{
	...
	switch (type)
	{
		case GE_OWNERSHIP_TAKE:
		{
			...
			m_trunk_items.push_back(id);
			itm->H_SetParent(smart_cast<CObject*>(this));
		...
		case GE_OWNERSHIP_REJECT:
		{
			...
			it = std::find(m_trunk_items.begin(), m_trunk_items.end(), id); VERIFY(it != m_trunk_items.end());
			m_trunk_items.erase(it);

			bool just_before_destroy = !P.r_eof() && P.r_u8();
			bool dont_create_shell = just_before_destroy;
...
void CCar::ChangeFuel(float f)
{
	m_fuel += f;
	if (m_fuel > m_fuel_tank)
		m_fuel = m_fuel_tank;
	if (m_fuel < 0)
		m_fuel = 0;
}
...
void CCanister::Load(LPCSTR section)
{
	inherited::Load(section);

	default_weight = pSettings->r_float(section, "inv_weight");
	volume = pSettings->r_float(section, "volume");
	SetFuel(volume);
}

void CCanister::SetFuel(float value)
{
	fuel = value > volume ? volume : value;
	if (fuel < 0)
		fuel = 0;
	m_weight = default_weight + (round(fuel * 100) / 100);
}

void CCanister::ChangeFuel(float value)
{
	fuel += value;
	if (fuel > volume)
		fuel = volume;
...
void CCanister::UpdateCL()
{
	inherited::UpdateCL();
}

void CCanister::Update()
{
}

void CCanister::save(NET_Packet &output_packet)
{
	inherited::save(output_packet);
	save_data(fuel, output_packet);
}

void CCanister::load(IReader &input_packet)
{
	inherited::load(input_packet);
	load_data(fuel, input_packet);
}

BOOL CCanister::net_Spawn(CSE_Abstract* DC)
{
	return		(inherited::net_Spawn(DC));
}

void CCanister::net_Destroy()
{
	inherited::net_Destroy();
}

void CCanister::OnH_A_Chield()
{
	inherited::OnH_A_Chield();
}

void CCanister::OnH_B_Independent(bool just_before_destroy)
{
	inherited::OnH_B_Independent(just_before_destroy);
}

void CCanister::renderable_Render()
{
	inherited::renderable_Render();
}


CCarToolkit::CCarToolkit()
{
}
...
void CCarToolkit::Load(LPCSTR section)
{
	inherited::Load(section);

	capacity = pSettings->r_float(section, "capacity");
	applying_remain = pSettings->r_float(section, "applying");
}

void CCarToolkit::UpdateCL()
{
	inherited::UpdateCL();
}

void CCarToolkit::Update()
{
}

void CCarToolkit::save(NET_Packet &output_packet)
{
	inherited::save(output_packet);
	save_data(applying_remain, output_packet);
}

void CCarToolkit::load(IReader &input_packet)
{
	inherited::load(input_packet);
	load_data(applying_remain, input_packet);
}

BOOL CCarToolkit::net_Spawn(CSE_Abstract* DC)
{
	return		(inherited::net_Spawn(DC));
}

void CCarToolkit::net_Destroy()
{
	inherited::net_Destroy();
}

void CCarToolkit::OnH_A_Chield()
{
	inherited::OnH_A_Chield();
}

void CCarToolkit::OnH_B_Independent(bool just_before_destroy)
{
	inherited::OnH_B_Independent(just_before_destroy);
}

void CCarToolkit::renderable_Render()
{
	inherited::renderable_Render();
}