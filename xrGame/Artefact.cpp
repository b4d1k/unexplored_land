...
void CArtefact::Load(LPCSTR section) 
{
	...
	m_WalkAccel = pSettings->r_float	(section,"walk_accel");
	m_JumpSpeed = pSettings->r_float	(section,"jump_speed");
	//end
	...
	{
		m_ArtefactHitImmunities.LoadAfsImmunities(pSettings->r_string(section,"hit_absorbation_sect"),pSettings);
	}
...
// skif : (уничтожение артефакта при хите)
void CArtefact::Hit(SHit *pHDS)
{
	if (pSettings->line_exist(this->cNameSect_str(),"dont_destroy_on_hit"))
		if (pSettings->r_bool(this->cNameSect_str(),"dont_destroy_on_hit") == TRUE)
			return;
	SHit& HDS	= *pHDS;
	if (HDS.hit_type == ALife::eHitTypeFireWound)
	{
		...
		/*if (pSettings->line_exist(this->cNameSect_str(),"destroy_particles"))
			p = pSettings->r_string(this->cNameSect_str(),"destroy_particles");
		else
			p = "anomaly2\\body_tear_00";*/

		if (pSettings->line_exist(this->cNameSect_str(),"destroy_snd"))
			snd = pSettings->r_string(this->cNameSect_str(),"destroy_snd");
		else
			snd = "anomaly\\bfuzz_blowout";
		...
		this->DestroyObject();
		/*Fvector dir;
		CParticlesPlayer* pp = smart_cast<CParticlesPlayer*>(this->PhysicsShellHolder());
		pp->StartParticles(p,dir.set(0,1,0),ID());
		destroy = true;*/
	}
}
// end
