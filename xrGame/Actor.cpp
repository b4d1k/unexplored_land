...
void CActor::Load	(LPCSTR section )
{
	...
	*/
	...
	m_fWalkAccelDefault			= m_fWalkAccel;
	m_fJumpSpeed				= pSettings->r_float(section,"jump_speed");
	m_fJumpSpeedDefault			= m_fJumpSpeed;
...
void CActor::Die	(CObject* who)
{
	...
	cam_Set				(eacFreeLook); // skif : (смерть от первого лица) - замена eacFreeLook на eacFirstEye
...
void CActor::g_PerformDrop	( )
{
	...
	//skif : (слоты)
	if(inventory().SlotIsPersistent(s) && s != KNIFE_SLOT && s != BINOCULAR_SLOT)	
		return;
	// end
...
#define ARTEFACTS_UPDATE_TIME 1.0f // skif
...
void CActor::UpdateArtefactsOnBeltAndOutfit()
{
	...
	float delta = 0.002f;
	float addWalkAccel = 0.0f;
	float addJumpSpeed = 0.0f;
	// end
		...
		inventory().m_belt.end() != it; ++it) 
	{
		...
		if(artefact)
		{
			conditions().ChangeBleeding		(delta / 100 * artefact->m_fBleedingRestoreSpeed);
			conditions().ChangeHealth		(delta / 100 * artefact->m_fHealthRestoreSpeed);
			conditions().ChangePower		(0.01f / 100 * artefact->m_fPowerRestoreSpeed);
			//skif : (доп свойства артефактов)
			conditions().ChangeSatietySpeed(delta / 100 * artefact->m_fSatietyRestoreSpeed);
			conditions().ChangePsyHealth	(delta / 100 * artefact->m_fPsyHealthRestoreSpeed);
			conditions().ChangeThirstSpeed(delta / 100 * artefact->m_fThirstRestoreSpeed);
			conditions().ChangeSleepSpeed	(delta / 100 * artefact->m_fSleepRestoreSpeed);
			addWalkAccel += artefact->m_WalkAccel;
			addJumpSpeed += artefact->m_JumpSpeed;
			//end
			conditions().ChangeRadiation(delta / 100 * artefact->m_fRadiationRestoreSpeed);
			/*if(artefact->m_fRadiationRestoreSpeed>0.0f) 
			{
				float val = (artefact->m_fRadiationRestoreSpeed/1000) - conditions().GetBoostRadiationImmunity();
				clamp(val, 0.0f, val);
				conditions().ChangeRadiation(val * f_update_time);
			}
			else
				conditions().ChangeRadiation((artefact->m_fRadiationRestoreSpeed/1000)	* f_update_time);*/
		}
	}
	...
	if (outfit)
	{
		conditions().ChangeBleeding(delta / 100 * outfit->m_fBleedingRestoreSpeed);
		conditions().ChangeHealth(delta / 100 * outfit->m_fHealthRestoreSpeed);
		conditions().ChangePower(0.01f / 100 * outfit->m_fPowerRestoreSpeed);
		//skif : (доп св-ва броников)
		conditions().ChangeSatietySpeed(delta / 100 * outfit->m_fSatietyRestoreSpeed);
		conditions().ChangePsyHealth(delta / 100 * outfit->m_fPsyHealthRestoreSpeed);
		conditions().ChangeThirstSpeed(delta / 100 * outfit->m_fThirstRestoreSpeed);
		conditions().ChangeSleepSpeed(delta / 100 * outfit->m_fSleepRestoreSpeed);
		//end
		conditions().ChangeRadiation(delta / 100 * outfit->m_fRadiationRestoreSpeed);
		//skif : (доп св-ва броников)
		addWalkAccel += outfit->m_WalkAccel;
		addJumpSpeed += outfit->m_JumpSpeed;
		// end
	}
	else
	{
	...
	{
		//skif : (доп св-ва броников)
		conditions().ChangeBleeding(delta / 100 * pHelmet->m_fBleedingRestoreSpeed);
		conditions().ChangeHealth(delta / 100 * pHelmet->m_fHealthRestoreSpeed);
		conditions().ChangePower(0.01f / 100 * pHelmet->m_fPowerRestoreSpeed);
		conditions().ChangeSatietySpeed(delta / 100 * pHelmet->m_fSatietyRestoreSpeed);
		conditions().ChangePsyHealth(delta / 100 * pHelmet->m_fPsyHealthRestoreSpeed);
		conditions().ChangeThirstSpeed(delta / 100 * pHelmet->m_fThirstRestoreSpeed);
		conditions().ChangeSleepSpeed(delta / 100 * pHelmet->m_fSleepRestoreSpeed);
		conditions().ChangeRadiation(delta / 100 * pHelmet->m_fRadiationRestoreSpeed);
		//end
	}
	// skif : (батарейка для фонарика)
	CTorch* pTorch = smart_cast<CTorch*>( inventory().ItemFromSlot(TORCH_SLOT) );
	CTorchBatarey* pTorchBt = smart_cast<CTorchBatarey*>( inventory().ItemFromSlot(TORCH_BATAREY_SLOT) );
			
	if (pTorch && pTorchBt)
	{
		if (pTorchBt->GetPower() == 0.f)
		{
			pTorch->Switch(false);
			pTorchBt->Active(false);
		}
		pTorchBt->Update();
	}
	// end
	//skif : (доп св-ва броников)
	m_fWalkAccel = m_fWalkAccelDefault + addWalkAccel;
	m_fJumpSpeed = m_fJumpSpeedDefault + addJumpSpeed;
	character_physics_support()->movement()->SetJumpUpVelocity(m_fJumpSpeed);
	// end
}
...
// skif - function is not important
float CActor::GetRestoreSpeed( ALife::EConditionRestoreType const& type )
{
	...
	case ALife::eHealthRestoreSpeed:
	{
		...
		for( ; itb != ite; ++itb ) 
		{
			...
			if ( artefact )
			{
				res += artefact->m_fHealthRestoreSpeed/10000;
			}
		}
		CCustomOutfit* outfit = GetOutfit();
		if ( outfit )
		{
			res += outfit->m_fHealthRestoreSpeed / 10000;
		}
		break;
	}
	...
	{	
		...
		for( ; itb != ite; ++itb ) 
		{
			...
			if ( artefact )
			{
				res += (artefact->m_fRadiationRestoreSpeed/1000);
			}
		}
		CCustomOutfit* outfit = GetOutfit();
		if ( outfit )
	...
	case ALife::eSatietyRestoreSpeed:
	{
		...
		for( ; itb != ite; ++itb ) 
		{
			...
			if ( artefact )
			{
				res += (artefact->m_fSatietyRestoreSpeed/1000);
			}
		}
		CCustomOutfit* outfit = GetOutfit();
		if ( outfit )
	...
	case ALife::ePowerRestoreSpeed:
	{
		...
		for( ; itb != ite; ++itb ) 
		{
			...
			if ( artefact )
			{
				res += (artefact->m_fPowerRestoreSpeed/1000);
			}
		}
		//skif : (доп св-ва броников)
		CCustomOutfit* outfit = GetOutfit();
		...
		if (outfit)
		{
			...
			if (outfit->m_fPowerLoss!=0.0f)
				res /= outfit->m_fPowerLoss;
		}
		if (helmet)
		{
			res += helmet->m_fPowerRestoreSpeed;
			if (helmet->m_fPowerLoss!=0.0f)
	...
	case ALife::eBleedingRestoreSpeed:
	{
		...
		for( ; itb != ite; ++itb ) 
		{
			...
			if ( artefact )
			{
				res += (artefact->m_fBleedingRestoreSpeed/1000);
			}
		}
		CCustomOutfit* outfit = GetOutfit();
		if ( outfit )
