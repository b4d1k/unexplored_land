...
//	HW.pContext->ClearDepthStencilView( RCache.get_ZB(), D3D_CLEAR_DEPTH|D3D_CLEAR_STENCIL, 1.0f, 0);
	...
	back_size.set					(_w/2,696.0f*k.y); // skif : (изменение загрузочного меню) - координаты для стадии загрузки
	...
	{
		Frect						r;
		r.lt.set					(255,367); // skif : (изменение загрузочного меню) - координаты для вставки intro текущей локации
		...
		r.lt.x						+= offs;
		r.lt.y						+= offs;
		back_size.set				(512,256); // skif : (изменение загрузочного меню) - размер intro текущей локации
		...
		logo_tex_coords.rb.set		(1.0f,1.0f);
