...
void CSE_ALifeCar::data_load(NET_Packet	&tNetPacket)
{
	...
	trunk_items.clear();
	u16 trunk_items_cnt =tNetPacket.r_u16();
	for(u16 i=0;i < trunk_items_cnt;++i)
	{
		trunk_items.push_back(tNetPacket.r_u16());
	}
}
void CSE_ALifeCar::data_save(NET_Packet &tNetPacket)
{
	...
	tNetPacket.w_float(health);
	{
		tNetPacket.w_u16(u16(trunk_items.size()));
		for (xr_vector<u16>::const_iterator it = trunk_items.begin(); trunk_items.end() != it; ++it)
		{
			tNetPacket.w_u16(*it);
		}
	}
